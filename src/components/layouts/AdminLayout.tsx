...
const AdminLayout = ({ children }: AdminLayoutProps) => {
  ...
  const menuItems = [
    { to: '/admin', label: 'Oversikt', icon: <LayoutDashboard className="h-5 w-5" /> },
    { to: '/admin/drift', label: 'Drift', icon: <FileSpreadsheet className="h-5 w-5" /> },
    { to: '/admin/ansatte', label: 'Ansatte', icon: <Users className="h-5 w-5" /> },
    { to: '/admin/arkiv', label: 'Arkiv', icon: <Archive className="h-5 w-5" /> },
    { to: '/admin/vedlikehold', label: 'Vedlikehold', icon: <Tool className="h-5 w-5" /> },
    { to: '/admin/innstillinger', label: 'Innstillinger', icon: <Settings className="h-5 w-5" /> },
  ];

  return (
    <div className="min-h-screen bg-background flex flex-col">
      {/* Top Navigation Bar */}
      <header className="sticky top-0 z-40 glass-effect border-b border-border/40 px-4 py-3">
